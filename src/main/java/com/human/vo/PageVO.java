...
public class PageVO {
	...
	// jsp단에서 값이 올때 웹에서 받은 페이지정보 (String)가 없으면 null 값인데 int는 null을 저장할 수 없음 오류방지위해
	// Integer (객체타입)사용
...
// 		DB쿼리에서 사용... 시작데이터번호 = (jsp클릭한 페이지번호-1) * 페이지당 보여지는 개수
		startNo = (this.page - 1) * perPageNum + 1; // 6페이지일 경우 시작글번호가 50부터..
...
//					 6		10	-	10 + 1 ->  1
//					11		20	-	10 + 1 -> 11
// 		시작 페이지 계산 클릭한 page번호 10일 때까지 시작페이지는 1
		
		if (tempEnd * this.perPageNum > this.totalCount) {
		// 클릭한 page번호로 계산된 게시물수가 실제게시물개수 totalCount보다 클 때
			...
			// 클릭한 page번호로 계산된 게시물수가 실제게시물개수 totalCount보다 작을때
